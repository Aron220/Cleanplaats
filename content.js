...
function unhideListingsByTerm(term) {
    ...
    document.querySelectorAll('.hz-Listing').forEach(listing => {
        const titleEl = listing.querySelector('.hz-Listing-group--title-description, hz-StructuredListing-body');
        if (titleEl && titleEl.textContent.toLowerCase().includes(term.toLowerCase())) {
...
function performCleanup() {
    ...
    document.querySelectorAll('.hz-Listing').forEach(listing => {
        const sellerNameEl = listing.querySelector('.hz-Listing-seller-name-new, .hz-Listing-seller-link, .hz-Listing-sellerName-new');
...
function removeTopAdvertisements() {
    ...
    const removedCount = findAndHideListings('.hz-Listing-priority-new', label);
...
function removeDagtoppers() {
    const removedCount = findAndHideListings('.hz-Listing-priority-new', 'Dagtopper');
...
function removeOpvalStickerListings() {
    ...
    const stickerSelectors = [
        '.hz-Listing-Opvalsticker-wrapper-new',
...
function injectBlacklistButtons() {
    document.querySelectorAll('.hz-Listing').forEach(listing => {
        ...
        // First try car advert structure (.hz-Listing-sellerName-new)
        const carSellerElement = listing.querySelector('.hz-Listing-sellerName-new');
        if (carSellerElement) {
            sellerName = carSellerElement.textContent.trim();
            sellerElement = carSellerElement;
            isCarAdvert = true;
        } else {
            // Try normal advert structure (.hz-Listing-seller-name-container-new)
            const sellerNameContainer = listing.querySelector('.hz-Listing-seller-name-container-new');
            if (sellerNameContainer) {
                const sellerLink = sellerNameContainer.querySelector('a');
                if (sellerLink) {
                    const sellerNameEl = sellerLink.querySelector('.hz-Listing-seller-name-new');
                    if (sellerNameEl) {
        ...
        if (window.innerWidth < 700) {
            ...
            const content = listing.querySelector('.hz-Listing-listview-content-new');
